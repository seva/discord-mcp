...
- **`DiscordClient`**:
  ...
  - Methods:
    ...
    - `search_guild_messages(guild_id: str, query: str, channel_id: str | None = None, limit: int = 10) -> list[dict]`: `GET /guilds/{guild_id}/messages/search?content={query}`; results flattened and **truncated client-side to `limit`** — the endpoint has no server-side limit param.
