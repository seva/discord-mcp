...
- **Topology ruling** (2026-09-22, Owner-accepted, Grok-verified): this server is an execution-plane **leaf** — Ichnos is the sole Funnel-exposed gateway/control plane; consolidation of MCP servers onto one shared process is rejected ("compose in the control plane, isolate in the execution plane"). HTTP `/mcp` stays localhost-bound until consumer auth exists (`docs/scope.md` terminal bound). Consumer-side integration (Ichnos adapter) lives in Ichnos's cycle — home: Ichnos channel-matrix + seva/Ichnos#9.
- **CLI Commands**:
