...
- **Declared deviation** (ARCHITECTURE.md banned pattern 2): CLI serve-dispatch tests patch at the transport-launch boundary rather than launching blocking transports in-process — the stdio path patches the FastMCP transport entry (`mcp.run`, tests/test_main.py:51), the HTTP path patches the server factory (`build_mcp`, tests/test_main.py:61); no tool or client logic is mocked anywhere in the suite (tool/client tests run through respx at the HTTP layer and the real DPAPI store path).
