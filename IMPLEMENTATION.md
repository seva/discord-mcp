...
- [x] HTTP transport (tests-first): `serve --transport http --port N` — FastMCP `build_mcp(host, port)` factory (module `mcp` = stdio instance); streamable-HTTP at `/mcp`, 127.0.0.1-bound by default (terminal bound). 3 new tests (CLI dispatch, factory pin, real subprocess HTTP round trip: initialize + list_tools). Live: `discord_status` over streamable-HTTP — real DPAPI auth + real Discord API.
